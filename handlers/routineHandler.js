...
class RoutineHandler {
  ...
  async handleRoutineCommand(interaction) {
    ...
    console.log('🎯 ルーティンコマンド実行:', { group, subcommand });
...
// === 履歴・統計ハンドラー ===
  ...
  async handleToday(interaction) {
    ...
    try {
      const executions = await this.routineService.getTodayRoutineExecutions(userId);
...
// === ユーティリティハンドラー ===
  ...
  async handleSearch(interaction) {
    ...
    try {
      const results = await this.routineService.searchRoutines(userId, keyword);
